...
use sea_orm::{ActiveValue::Set, DatabaseConnection, DeleteResult, EntityTrait, ActiveModelTrait};

use crate::{commands::create_record::CreateRecord, errors::errors::RepositoryError, repositories::{crud_repo::CrudRepo, measurement_point_repo::MeasurementPointRepo}, structs::record::Record};
...
impl CrudRepo<Record, CreateRecord, i32> for RecordRepo{
    async fn find_by_id(&self, id: i32)-> Result<Record, RepositoryError>{
        ...
        records::Entity::find_by_id(id)
            ...
            .all(&self.db)
            .await?;
        let (record_model, points_models)  : (records::Model,Vec<measurement_points::Model>)= match record_with_measurement_points.first(){
    ...
    async fn insert(&self, new_record: CreateRecord) -> Result<Record, RepositoryError>{
        ...
        self.measurement_point_repo.insert_many(new_record.measurement_points, record_model.record_id).await?;
