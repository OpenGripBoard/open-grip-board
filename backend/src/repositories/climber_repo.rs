...
use crate::errors::{AuthentificationError, RepositoryError};
...
impl ClimberRepo {
    pub async fn authenticate_climber_password(
        &self,
        email: String,
        password: String,
    ) -> Result<Option<Climber>, AuthentificationError> {
    ...
    async fn get_password_hash(&self, email: String) -> Result<Option<String>, RepositoryError> {
        ...
        Ok(climber_model.password_hash)
    }
    ...
    pub async fn insert_favourite_gyms_relation(
        &self,
        climber_id: i32,
        gym_id: i32,
    ) -> Result<(), RepositoryError> {
        let favourite_gyms = favourite_gyms::ActiveModel {
            climber_id: Set(climber_id),
            gym_id: Set(gym_id),
        };
    ...
    pub async fn delete_favourite_gyms_relation(
        &self,
        climber_id: i32,
        gym_id: i32,
    ) -> Result<(), RepositoryError> {
